...
```
^button-g0kw
note quotidienne du jour précédent : [[<% tp.date.now("YYYY-MM-DD", -1, tp.file.title, "YYYY-MM-DD") %>]]
___
# éléments reportés
```tasks
...
```dataview
table
from #<% tp.date.now("YYYY-MM-DD", 0, tp.file.title, "YYYY-MM-DD") %> and -"Notes Quotidiennes"
```
```button
name Rafraîchir
type line(38) text
